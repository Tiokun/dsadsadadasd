...
void KitParser::Dump()
{
	...
	std::ofstream skinso("C:\\black_panties\\dump\\skins.json");
	if (skinso.good())
		skinso << std::setw(4) << skins << std::endl;

	json gloves(map_gloves);

	std::ofstream gloveso("C:\\black_panties\\dump\\gloves.json");
	if (gloveso.good())
		gloveso << std::setw(4) << gloves << std::endl;

	json stickers(map_stickers);

	std::ofstream stickero("C:\\black_panties\\dump\\stickers.json");
	if (stickero.good())
